...
const ChromaGrid = ({ items, className = '', radius = 300, damping = 0.45, fadeOut = 0.6, ease = 'power3.out' }) => {
  ...
  return (
    <div
      ...
      className={`relative w-full min-h-screen gap-2 flex flex-wrap justify-center items-start gap-3 ${className}`}
      style={{
        '--r': `${radius}px`,
        '--x': '50%',
        '--y': '50%'
    ...
    >
      {data.map((c, i) => (
        ...
        >
          <div
            className="absolute inset-0 pointer-events-none bg-transparent transition-opacity duration-500 z-20 opacity-0 group-hover:opacity-100"
            style={{
              background:
